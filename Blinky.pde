class Blinky {
  ...
  PVector pos = tileToPixel(new PVector(13, 11));//starting position
  ...
  void setPathNodes() {
    ...
    if (returnHome) {//if returning home then the target is just above the ghost room thing
      ...
      if (chase) {
        ghostPathNodes.add(new PathNode(pixelToTile(pacman.pos)));//target pacman
      } else {
        ghostPathNodes.add(new PathNode(1, 1));//scatter to corner
      }
    }
